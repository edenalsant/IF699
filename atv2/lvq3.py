...
def lvq3(X, S, learning_rate, adjustment, y):
    rand.seed(None)
        ...
        if is_at_window(p,ei,ej,w):
            if y.iloc[ei_index] != y.iloc[ej_index]:
                if y.iloc[ei_index] == p_label:
                    ...
                    ei = ei - learning_rate * (p-ei)
                    ej = ej + learning_rate * (p-ej)
            elif y.iloc[ei_index] == y.iloc[ej_index] == p_label:
                ei = ei + learning_rate * (p-ei)
                ej = ej + learning_rate * (p-ej)
...
def main():
    ...
    S = lvq3(X, Sx, learning_rate, adjustment, Sy)
    S.to_csv('./lvq_output/seed_137/kc1_lvq3.csv')
